...
async function main() {
  ...
  if (['pro', 'reasoning'].includes(chosen)) {
    const { PerplexityClient } = await import('../src/index.js');
  ...
  try {
    ...
    if (fs.existsSync('perplexity_cookies.json')) {
      ...
      client = new PerplexityClient({ cookies: cookieHeader });
  ...
  while (true) {
    ...
    try {
      ...
      const res = await client.search(q, { mode: chosen });
      console.log('Answer:');
    ...
    if (Array.isArray(res.text)) {
      const chunks = res.text.flatMap(t => {
        if (t && t.content && typeof t.content === 'object') {
          if (t.content.answer) {
            try { return [JSON.parse(t.content.answer).answer]; } catch (e) { return [t.content.answer]; }
          }
        }
        return [];
      }).filter(Boolean);
